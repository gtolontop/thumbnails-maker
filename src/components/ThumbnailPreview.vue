<template>
  ...
  >
    <div v-if="!isTextOnly" id="default-layout" class="flex justify-between items-center w-full h-full">
      <div class="left-panel flex flex-col justify-center h-full w-1/2 relative -left-10">
        ...
        <div class="title-container relative">
          <h1 class="title text-[90px] font-extrabold leading-[1] m-0" style="color: #9EA4BE;">{{ configStore.titleText }}</h1>
          <div class="reflect-wrapper absolute left-0 w-full overflow-hidden" style="top: calc(100% - 22px); height: 90px;">
            <h1 class="text-[90px] font-extrabold leading-[1] m-0 transform scale-y-[-1] opacity-30" 
                style="color: #9EA4BE;
      ...
      <div class="right-panel flex flex-col items-center justify-start h-full w-1/2 relative -right-10">
        <div class="right-panel-header mb-6 flex items-center justify-center gap-6">
          ...
          <div v-if="configStore.headerLinkText" class="inline-block">
            <div class="px-6 py-2 rounded-lg" style="background-color: #3a3f4d;">
    ...
    <div v-if="isTextOnly" id="text-only-layout" class="w-full h-full relative">
      <div id="text-only-main-content" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full text-center">
        ...
        <div class="relative inline-block">
          <h1 id="text-only-title" class="text-[110px] font-extrabold leading-[1] m-0" style="color: #9EA4BE;">{{ configStore.titleText }}</h1>
          <div class="absolute left-0 w-full overflow-hidden" style="top: calc(100% - 27px); height: 110px;">
            <h1 class="text-[110px] font-extrabold leading-[1] m-0 transform scale-y-[-1] opacity-30" 
                style="color: #9EA4BE;
                       background: linear-gradient(to bottom, transparent 20%, #9EA4BE 100%); 
                       -webkit-background-clip: text; 
                       -webkit-text-fill-color: transparent;
